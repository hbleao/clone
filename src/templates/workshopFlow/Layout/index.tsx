'use client';
import { Grid } from '@porto-ocean/grid';
import { Row } from '@porto-ocean/row';
import Image from 'next/image';

import './styles.scss';

import { usePageStore } from '@/store';

import { HeaderStore, Link } from '@/components';
import * as Stepper from '@/components/Stepper';

import { getStepParam } from '@/utils';

import type { WorkshopFlowProps } from './types';

export const WorkshopFlowLayout = ({ children }: WorkshopFlowProps) => {
	const page = usePageStore((state) => state.page);
	const step = getStepParam(page.steps);
	const currentStep = step?.index || 0;
	const stepIndex = currentStep as number;
	const nextLabel = page?.steps[stepIndex]?.nextLabe;
	const isNextStep = page?.steps[stepIndex + 1];
	const nextStepLabel = nextLabel ?? isNextStep?.label ?? '';

	const tagStep = {
		'0': {
			name: 'oficina',
			subname: 'selecione-a-oficina',
		},
		'1': {
			name: 'agendamento',
			subname: 'selecione-o-dia-e-horário',
		},
		'2': {
			name: 'dados-cadastrais',
			subname: 'informe-seus-dados',
		},
	};

	if (!step) return <></>;

	return (
		<section className="workshop-flow">
			<Grid>
				<Row landscape={[3, 11]} desktop={[3, 11]} wide={[3, 11]}>
					<HeaderStore />
				</Row>
				{!!step?.label && (
					<>
						<Row landscape={[3, 11]} desktop={[3, 11]} wide={[3, 11]}>
							<div className="workshop-flow__button-back">
								<Link
									href={step?.backLink}
									variant="insurance"
									size="small"
									styles="ghost"
									data-gtm-type="click"
									data-gtm-clicktype="button"
									data-gtm-name={`porto-servico:${tagStep[stepIndex].name}`}
									data-gtm-subname={`${tagStep[stepIndex].subname}:voltar`}
								>
									<Image
										src="data:image/png;base64,"
										alt=""
										width={13}
										height={13}
									/>
									Voltar
								</Link>
							</div>
							<Stepper.Root className="workshop-flow__stepper">
								<Stepper.Header>
									<Stepper.Legend>
										<strong>{`${stepIndex + 1} de ${
											page.steps?.length
										} `}</strong>
										{` - ${step?.label}`}
									</Stepper.Legend>
									<Stepper.Legend className="workshop-flow__stepper-next-step">
										{nextStepLabel && `Próximo passo: ${nextStepLabel}`}
									</Stepper.Legend>
								</Stepper.Header>
								<Stepper.Body>
									{page.steps.map((currentStep, index) => (
										<Stepper.Step
											key={currentStep.label}
											isActive={index === stepIndex}
											isCompleted={index < stepIndex}
										/>
									))}
								</Stepper.Body>
							</Stepper.Root>
						</Row>
						<Row landscape={[3, 11]} desktop={[3, 11]} wide={[3, 11]}>
							<div className="workshop-flow__layout">{children}</div>
						</Row>
					</>
				)}
			</Grid>
		</section>
	);
};
